...
export default function ClientePrenotazione() {
            ...
            <>
                ...
                {data.ordinazioni.map(ordinazione => (
                    <tr key={ordinazione.id}>
                        ...
                        <td><button onClick={() => deleteOrdinazione(ordinazione.id)} className="btn btn-block">Cancella</button></td>
                    </tr>
                ))}
                </tbody>
            </>
    ...
    const deleteOrdinazione = async (id) => {

        if (!window.confirm("Sei sicuro di voler eliminare questa ordinazione?"))
            return;

        try {
            const response = await axiosClient.delete(`/ordinazione/${id}`);
            setNotificationStatus('success');
