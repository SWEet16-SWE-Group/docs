...
\begin{itemize}
  \item \textbf{\LaTeX:} Linguaggio di markup per la preparazione di testi, basato sul programma di composizione tipografica TEX;
    ...
    \end{center}
  \item \textbf{\LaTeX WorkShop:} Estensione per VSCode che include un compilatore integrato, l'autocompletamento dei comandi \LaTeX  e la segnalazione di errori di sintassi. \\
  L'estensione è poi configurata in sinergia con Git$^{G}$ per ignorare la presenza di eventuali artefatti binari risultati dalla compilazione;
  \begin{center}
    \url{https://github.com/James-Yu/LaTeX-Workshop}
  \end{center}
  \item \textbf{StarUML:} Software di disegno grafico multipiattaforma per la produzione di diagrammi UML$^{G}$. \\
  Il gruppo ha scelto questo strumento perché gratuito (anche se con qualche limitazione) e perché comprende tutte le funzionalità necessarie al nostro utilizzo.
  \begin{center}
